...
- **`Select.onValueChange` is `(value: string | null) => void`.** Radix's callback was `(value: string) => void`; Base UI allows `null` (likely when a selection is cleared). Setters typed as `Dispatch<SetStateAction<string>>` reject `null` at build time. Coalesce at the boundary: `onValueChange={(v) => setField(v ?? "")}`. Surfaced in session 4 catering forms.
- **`Select` doesn't emit a native form field.** For `<form action={serverAction}>` flows, pair each controlled `Select` with a hidden `<input name={field} value={state}>` so the value lands in the FormData the server action receives. Same workaround holds for any Base UI primitive that's controlled-only (most of them). Surfaced in session 4 catering forms.
- **Dialog / Sheet / Drawer use a `render` prop for composition, not `asChild`.** Base UI's Dialog family (which shadcn's `sheet` wraps via `@base-ui/react/dialog`) composes via `<Trigger render={<Button />} />` rather than Radix's `<Trigger asChild><Button /></Trigger>`. When you need a styled trigger button, either use `render`, or — usually simpler — bypass the Trigger component entirely and drive `open` state externally via a plain `onClick`. Default `SheetContent` width is `w-3/4 sm:max-w-sm` (narrower than Radix's default); override via `className="w-full sm:max-w-lg"` for a typical cart-drawer width. Surfaced in session 5b cart drawer.
